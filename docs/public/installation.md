...
The `kafka.config` parameter lets you specify Apache Kafka broker properties using their **native names** directly, without manually encoding them as `CONF_KAFKA_*` environment variables.
...
Each key is the exact Kafka property name. The operator translates entries to the equivalent `CONF_KAFKA_*` environment variables internally, so the existing `docker-entrypoint.sh` behaviour is unchanged.

**Precedence:** When the same property is specified in both `kafka.config` and `kafka.environmentVariables`, the `environmentVariables` value is used and the `config` value is ignored. This lets you selectively override individual properties without modifying `kafka.config`.

```yaml
kafka:
  config:
    num.io.threads: "8"           # overridden by environmentVariables below
    log.retention.hours: "168"    # applied from config
  environmentVariables:
    - CONF_KAFKA_NUM_IO_THREADS=32  # wins over kafka.config for this property
```

**Backward compatibility:** Deployments that do not set `kafka.config` are fully unaffected. The field is optional and defaults to an empty map.
...
| kafka.config                                           | map     | no        | {}                            | Native Kafka broker configuration properties. Keys are the exact Apache Kafka property names (e.g. `num.io.threads`) and values are strings. Each entry is passed verbatim to `server.properties`. When the same property is specified in both `kafka.config` and `kafka.environmentVariables`, the `environmentVariables` value takes effect. Deployments that do not set this parameter are unaffected. |
...
Consumer lag alerts require Kafka Exporter (`monitoring.lagExporter.enabled: true`). Global thresholds are configured with
`monitoring.thresholds.lagAlert` and `monitoring.thresholds.lagAlertSeconds`. Additional alerts with topic and consumer group
filters are configured with `monitoring.thresholds.lagAlertConfiguration`.
...
| `lagAlertSeconds` | integer | no        | `-1`    | Estimated lag seconds threshold. Set to `-1` to skip the seconds alert for this entry.         |
...
| mirrorMaker.config                       | map     | no        | {}                       | Native MirrorMaker (Kafka Connect / MM2) configuration properties. Keys are the exact property names (e.g. `refresh.topics.interval.seconds`) and values are strings. Each entry is passed verbatim to `mm2.properties` as a common (unscoped) property. When the same property is specified in both `mirrorMaker.config` and `mirrorMaker.environmentVariables`, the `environmentVariables` value takes effect. Scoped properties (per-cluster or per-flow) must still use `mirrorMaker.environmentVariables`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| integrationTests.atpReport.enabled                | boolean | no        | false                    | Opt-in for ATP report upload. When false, ATP S3 env vars are not passed to the pod.                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| integrationTests.atpReport.atpStorage.provider    | string  | no        | aws                      | S3 provider type for ATP Storage (for example `aws`, `minio`, `s3`). Used together with other atpStorage fields for S3-compatible upload of integration test reports.                                                                                                                                                                                                                                                                                                                                                                |
...
| integrationTests.atpReport.atpStorage.username    | string  | no        | ""                       | S3 access key. Sensitive; when report upload is enabled, values may be stored in a Kubernetes Secret.                                                                                                                                                                                                                                                                                                                                                                                                                                |
| integrationTests.atpReport.atpStorage.password    | string  | no        | ""                       | S3 secret key. Sensitive; when report upload is enabled, values may be stored in a Kubernetes Secret.                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
It can be enabled with `kafka.rollingUpdate: true`, by default it is enabled.
...
### Native MirrorMaker Configuration Properties

The `mirrorMaker.config` parameter lets you specify common MirrorMaker 2 (Kafka Connect / MM2) properties using their **native names** directly, without encoding them as `CONF_*` environment variables.
...
**Precedence:** When the same property is specified in both `mirrorMaker.config` and `mirrorMaker.environmentVariables`, the `environmentVariables` value is used and the `config` value is ignored.

**Scope:** `mirrorMaker.config` covers only **common (unscoped)** properties. Per-cluster properties (`<clusterAlias>_CONF_*`) and per-flow properties (`<source>_<target>_CONF_*`) must still be specified via `mirrorMaker.environmentVariables`.

**Backward compatibility:** Deployments that do not set `mirrorMaker.config` are fully unaffected.
