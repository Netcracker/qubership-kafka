...
| kafka_consumergroup_lag                            | Consumer group lag per topic/partition (Kafka Exporter)                                                         |                 |   Supported   | Not Supported |        Supported        |
...
| KafkaLagAlert                           | Some of Kafka Service pods have partition lag higher than specified limit                              | 3m  | high     | `max(kafka_consumergroup_lag{namespace="kafka-service"}) > limit`                                                                                                                                                                                     | Some of Kafka Service pods have partition lag higher than specified limit                                                                | [KafkaLagAlert](/docs/public/troubleshooting.md#lag-limit-reached)                                        | Supported     | Not Supported |
| KafkaEstimatedLagSecondsAlert           | Estimated consumer lag seconds exceeded threshold                                                        | 3m  | high     | `sum by (consumergroup) (kafka_consumergroup_lag{namespace="kafka-service"}) / clamp_min(sum(rate(kafka_topic_partition_current_offset{namespace="kafka-service"}[5m])), 1) > limit`                                                                         | Estimated lag seconds for consumer group exceeded threshold                                                                              | [KafkaEstimatedLagSecondsAlert](/docs/public/alerts.md#kafkaestimatedlagsecondsalert)                     | Supported     | Not Supported |
| Custom lag alerts                       | Offset lag or estimated lag seconds exceeded threshold for filtered topics/groups                      | 3m  | high     | Same expressions as `KafkaLagAlert` / `KafkaEstimatedLagSecondsAlert` with `consumergroup=~"<groups>"` and `topic=~"<topics>"` filters                                                                                                                        | Per-entry alert name from `monitoring.thresholds.lagAlertConfiguration`                                                                  | [Custom Consumer Lag Alerts](/docs/public/alerts.md#custom-consumer-lag-alerts)                           | Supported     | Not Supported |
