...
def __configure_logging(log):
    ...
    log_handler = RotatingFileHandler(filename='/opt/kafka-mirror-maker-monitoring/exec-scripts/health_metric.log',
                                      maxBytes=50 * 1024,
                                      backupCount=5)
    log_handler.setFormatter(formatter)
    log_handler.setLevel(logging.DEBUG if os.getenv('KAFKA_MIRROR_MAKER_MONITORING_SCRIPT_DEBUG') else logging.INFO)
    log.addHandler(log_handler)
    err_handler = RotatingFileHandler(filename='/opt/kafka-mirror-maker-monitoring/exec-scripts/health_metric.err',
                                      maxBytes=50 * 1024,
                                      backupCount=5)
